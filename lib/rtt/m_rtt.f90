MODULE m_rtt
  ...
  ! variables below are declared as threadprivate because this module is called from a parallelized region (otherwise they are seen
  ! as shared). Comment out if called from a serial region. Note that only isotropic scattering is covered here.
  !$OMP THREADPRIVATE (e0_p, e0_s, alpha, beta, veta_pp, veta_ss, veta_ps, veta_sp)
...
#else
    SUBROUTINE rtt_isotropic(time, tp, ts, eta_pp, eta_ps, eta_sp, eta_ss, bi, vs, wp, ws, tau, direct, envelope, ok)
#endif
...
#endif
      REAL(r32),    DIMENSION(:),           INTENT(IN)  :: time
      REAL(r32),                            INTENT(IN)  :: tp, ts, eta_pp, eta_ps, eta_sp, eta_ss, bi, vs, wp, ws, tau
      REAL(r32),    DIMENSION(2),           INTENT(OUT) :: direct
      REAL(r32),    DIMENSION(SIZE(time)),  INTENT(OUT) :: envelope
      INTEGER(i32),                         INTENT(OUT) :: ok
      COMPLEX(r64), DIMENSION(nfft/2+1)                 :: spectrum
