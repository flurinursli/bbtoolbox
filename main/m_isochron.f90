MODULE m_isochron
  ...
  CONTAINS
    ...
    SUBROUTINE solve_isochron_integral(ok, pl, vel, iter)
      ...
      !               ENDIF
      !
      !               attenuation = EXP(-avepath * eta / 2._r32)        !< attenuation factor direct wave (amplitude, not energy)
      !
      !               DO ic = 1, 3
      !
      !                 g31 = g(ic, 3) - g(ic, 1)
      ...
      !                   dl = HYPOT(du, dv)
      !
      !                   triseis(it, ic, band) = triseis(it, ic, band) + c * (ga + gb) * dl * 0.5_r32 * attenuation
      !
